import { disablePaginationButtons,authAction, renderPagination, setupJump, displayALL } from './export.js';

document.addEventListener('DOMContentLoaded', function() {
  const itemList = document.getElementById('volumes');
  ...
  searchButton.addEventListener('click', () => {
      ...
      if (searchTerm) {
          ...
          currentPage = 1; 
          fetchvolumes(currentSearchTerm);
      } else {
          alert("Please enter an volume name to search.");
      }
  });

  async function fetchvolumes(searchTerm = '') {
      const loadingIndicator = document.getElementById('loading');
      loadingIndicator.style.display = 'block';
      disablePaginationButtons(true);

      const offset = (currentPage * 100) - 100;
      let url = `https://comicvine.gamespot.com/api/volumes/?format=json&offset=${offset}&api_key=${API_KEY}&field_list=name,image,api_detail_url`;
