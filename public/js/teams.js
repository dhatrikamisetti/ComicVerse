...
document.addEventListener('DOMContentLoaded', function() {
  const itemList = document.getElementById('teams');
  ...
  searchButton.addEventListener('click', () => {
      ...
      if (searchTerm) {
          ...
          currentPage = 1; 
          fetchteams(currentSearchTerm);
      } else {
          alert("Please enter an team name to search.");
      }
  });

  async function fetchteams(searchTerm = '') {
      const loadingIndicator = document.getElementById('loading');
      loadingIndicator.style.display = 'block';
      disablePaginationButtons(true);

      const offset = (currentPage * 100) - 100;
      let url = `https://comicvine.gamespot.com/api/teams/?format=json&offset=${offset}&api_key=${API_KEY}&field_list=name,image,api_detail_url`;
