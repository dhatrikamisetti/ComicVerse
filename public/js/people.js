...
document.addEventListener('DOMContentLoaded', function() {
  const itemList = document.getElementById('people');
  ...
  searchButton.addEventListener('click', () => {
      ...
      if (searchTerm) {
          ...
          currentPage = 1; 
          fetchpeople(currentSearchTerm);
      } else {
          alert("Please enter an person name to search.");
      }
  });
  ...
  async function fetchpeople(searchTerm = '') {
      ...
      let url = `https://comicvine.gamespot.com/api/people/?format=json&offset=${offset}&api_key=${API_KEY}&field_list=name,image,api_detail_url`;
      ...
      try {
          ...
          alert('Failed to fetch people. Please try again later.');
      } finally {
