...
document.addEventListener('DOMContentLoaded', function() {
  const itemList = document.getElementById('objects');
  ...
  searchButton.addEventListener('click', () => {
      ...
      if (searchTerm) {
          ...
          currentPage = 1; 
          fetchobjects(currentSearchTerm);
      } else {
          alert("Please enter an object name to search.");
      }
  });

  async function fetchobjects(searchTerm = '') {
      const loadingIndicator = document.getElementById('loading');
      loadingIndicator.style.display = 'block';
      disablePaginationButtons(true);

      const offset = (currentPage * 100) - 100;
      let url = `https://comicvine.gamespot.com/api/objects/?format=json&offset=${offset}&api_key=${API_KEY}&field_list=name,image,api_detail_url`;
      ...
      try {
          ...
          alert('Failed to fetch objects. Please try again later.');
      } finally {
