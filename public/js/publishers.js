...
document.addEventListener('DOMContentLoaded', function() {
  const itemList = document.getElementById('publishers');
  ...
  searchButton.addEventListener('click', () => {
      ...
      if (searchTerm) {
          ...
          currentPage = 1; 
          fetchpublishers(currentSearchTerm);
      } else {
          alert("Please enter an publisher name to search.");
      }
  });

  async function fetchpublishers(searchTerm = '') {
      const loadingIndicator = document.getElementById('loading');
      loadingIndicator.style.display = 'block';
      disablePaginationButtons(true);

      const offset = (currentPage * 100) - 100;
      let url = `https://comicvine.gamespot.com/api/publishers/?format=json&offset=${offset}&api_key=${API_KEY}&field_list=name,image,api_detail_url`;
      ...
      try {
          ...
          alert('Failed to fetch publishers. Please try again later.');
      } finally {
